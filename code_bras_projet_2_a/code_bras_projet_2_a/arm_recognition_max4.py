#commenter
from reachy_sdk import ReachySDK
import cv2 as cv
import mediapipe as mp
import os
import numpy as np

path = 'C:/Users/maxbu/dossPython/liste_Reachy/recognised_images'
folder = 'camera_opencv' # or 'camera_images'

#reachy = ReachySDK(host='10.117.255.255')
left_shoulder_coords = []
right_shoulder_coords = []
left_elbow_coords = []
right_elbow_coords = []
left_wrist_coords = []
right_wrist_coords = []
left_hip_coords = []
right_hip_coords = []

compteur_image = 100

########################################################################################################################
# Invert the y coordinates of the landmarks
def invert_y_coords(coords):
    # for i in range(0, len(coords)):
    coords = 480 - coords
    return coords

########################################################################################################################
def arm_recognition(compteur_image):
    mp_pose = mp.solutions.pose
    pose = mp_pose.Pose()
    mpDraw = mp.solutions.drawing_utils

    for i in range(0, compteur_image):
        body_image = cv.imread('C:/Users/maxbu/dossPython/liste_Reachy/'+ folder +'/img_'+str(i)+'.jpg')
        imgRGB = cv.cvtColor(body_image, cv.COLOR_BGR2RGB)
        results = pose.process(imgRGB)

        lm = results.pose_landmarks
        lmPose = mp_pose.PoseLandmark
##############################################################################################################
        #Acquiring the coordinates of the landmarks

        # left shoulder
        left_shoulder_x = int(lm.landmark[lmPose.LEFT_SHOULDER].x * body_image.shape[1])
        left_shoulder_y = int(lm.landmark[lmPose.LEFT_SHOULDER].y * body_image.shape[0])
        left_shoulder_coords.append([left_shoulder_x, invert_y_coords(left_shoulder_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/left_shoulder_coords.txt', left_shoulder_coords,delimiter=", ", fmt='%s')

        # right shoulder
        right_shoulder_x = int(lm.landmark[lmPose.RIGHT_SHOULDER].x * body_image.shape[1])
        right_shoulder_y = int(lm.landmark[lmPose.RIGHT_SHOULDER].y * body_image.shape[0])
        right_shoulder_coords.append([right_shoulder_x, invert_y_coords(right_shoulder_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/right_shoulder_coords.txt', right_shoulder_coords,delimiter=", ", fmt='%s')

        # left elbow
        left_elbow_x = int(lm.landmark[lmPose.LEFT_ELBOW].x * body_image.shape[1])
        left_elbow_y = int(lm.landmark[lmPose.LEFT_ELBOW].y * body_image.shape[0])
        left_elbow_coords.append([left_elbow_x, invert_y_coords(left_elbow_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/left_elbow_coords.txt', left_elbow_coords,delimiter=", ", fmt='%s')

        # right elbow
        right_elbow_x = int(lm.landmark[lmPose.RIGHT_ELBOW].x * body_image.shape[1])
        right_elbow_y = int(lm.landmark[lmPose.RIGHT_ELBOW].y * body_image.shape[0])
        right_elbow_coords.append([right_elbow_x, invert_y_coords(right_elbow_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/right_elbow_coords.txt', right_elbow_coords,delimiter=", ", fmt='%s')

        # left wrist
        left_wrist_x = int(lm.landmark[lmPose.LEFT_WRIST].x * body_image.shape[1])
        left_wrist_y = int(lm.landmark[lmPose.LEFT_WRIST].y * body_image.shape[0])
        left_wrist_coords.append([left_wrist_x, invert_y_coords(left_wrist_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/left_wrist_coords.txt', left_wrist_coords,delimiter=", ", fmt='%s')

        # right wrist
        right_wrist_x = int(lm.landmark[lmPose.RIGHT_WRIST].x * body_image.shape[1])
        right_wrist_y = int(lm.landmark[lmPose.RIGHT_WRIST].y * body_image.shape[0])
        right_wrist_coords.append([right_wrist_x, invert_y_coords(right_wrist_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/right_wrist_coords.txt', right_wrist_coords,delimiter=", ", fmt='%s')

        # left hip
        left_hip_x = int(lm.landmark[lmPose.LEFT_HIP].x * body_image.shape[1])
        left_hip_y = int(lm.landmark[lmPose.LEFT_HIP].y * body_image.shape[0])
        left_hip_coords.append([left_hip_x, invert_y_coords(left_hip_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/left_hip_coords.txt', left_hip_coords,delimiter=", ", fmt='%s')

        # right hip
        right_hip_x = int(lm.landmark[lmPose.RIGHT_HIP].x * body_image.shape[1])
        right_hip_y = int(lm.landmark[lmPose.RIGHT_HIP].y * body_image.shape[0])
        right_hip_coords.append([right_hip_x, invert_y_coords(right_hip_y)])
        np.savetxt('C:/Users/maxbu/dossPython/liste_Reachy/liste/right_hip_coords.txt', right_hip_coords,delimiter=", ", fmt='%s')


##############################################################################################################
        # Drawing the landmarks on the images
        mpDraw.draw_landmarks(body_image, lm, mp_pose.POSE_CONNECTIONS, landmark_drawing_spec=mp.solutions.drawing_styles.get_default_pose_landmarks_style())
        cv.imwrite(os.path.join(path, 'body_recognise_'+str(i)+'.jpg'), body_image)
        cv.waitKey(1)

##############################################################################################################





arm_recognition(compteur_image)

